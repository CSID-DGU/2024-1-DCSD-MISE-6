...
def remove_rows_with_strings(df, column_name, strings_to_remove):
    """
    데이터 프레임에서 특정 문자열이 있으면 행을 제거

    Parameters:
    df : 처리할 데이터 프레임.
    column_name : 확인할 열
    strings_to_remove (list of str): 제거할 문자

    output:
    데이터 프레임
    """
    for string in strings_to_remove:
        df = df[~df[column_name].str.contains(string, regex=False)]
    return df